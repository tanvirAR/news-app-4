...
import SearchedNewsList from "./SearchedNewsList";

export default function NewsList() {
  const { category } = useGetCategory();
  const { searchQuery } = useGetSearchQuery();
  const { news, isLoading, isError } = useNewsQuery({ category });
  ...
  if (news && !isError && !isLoading) {
    const [left, right] = divideNews(news?.articles);
